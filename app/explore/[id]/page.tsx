...
export default function AttractionDetailPage() {
  ...
  const avg =
    attraction.reviews.length > 0
      ? attraction.reviews.reduce((s, r) => s + r.rating, 0) / attraction.reviews.length
      : 0;
  const checkedIn = checkins.includes(attraction.id);

  const handleCheckIn = () => {
    if (checkedIn) return;
    setCheckins((c) => [...c, attraction.id]);
      ...
      >
        ...
        <div className="space-y-5 p-5 sm:p-7">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {infoBlocks.map((b) => (
              <div key={b.label} className="rounded-xl border border-black/5 bg-black/[0.02] p-4 dark:border-white/10 dark:bg-white/[0.03]">
                ...
                <p className="text-sm leading-relaxed opacity-85">{b.body}</p>
              </div>
            ))}
          </div>
