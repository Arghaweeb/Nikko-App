...
export function HotelDetailClient({ id }: { id: string }) {
  ...
  const avg = hotel.reviews.reduce((sum, review) => sum + review.rating, 0) / Math.max(1, hotel.reviews.length);
        ...
        <div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[1.6fr_1fr]">
          ...
          <aside className="h-fit rounded-xl border border-black/5 bg-black/[0.02] p-4 dark:border-white/10 dark:bg-white/[0.03]">
            ...
            <div className="space-y-2">
              {hotel.bookingLinks.map((bookingLink) => (
                <a
                  key={bookingLink.label}
                  href={bookingLink.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="btn btn-ghost w-full justify-between"
                >
                  {bookingLink.label === "Official Website" ? t("hotels.official") : bookingLink.label}
